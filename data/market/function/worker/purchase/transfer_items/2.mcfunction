...
$loot insert 0 301 0 loot {pools:[{rolls:1,entries:[{type:"item",name:"$(item1_id)","weight":1,"functions":[{"function":"set_count","count":$(item1_count)}]}]}]}
...
execute if data storage pm:temp MarketWithdraw.item1_components run function market:worker/purchase/transfer_items/set_components/3 with storage pm:temp MarketWithdraw
